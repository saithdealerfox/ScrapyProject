...
for i in range(0,num_customers):
    ...
    try:
        ...
        WebDriverWait(div_activities[0],10).until(EC.element_to_be_clickable((By.XPATH,"//div[@class='activity--history--summary activity--history--110 clickable']")))
        div_tags = div_activities[0].find_elements_by_xpath("//div[@class='activity--history--summary activity--history--110 clickable']")
    ...
    if source_detail not in result_sold[date_input].keys():
        result_sold[date_input][source_detail] = {'C': 1}
        result_leads[date_input][source_detail] ={'L':0}
    else:
        result_sold[date_input][source_detail]['C']+=1
        result_leads[date_input][source_detail]['C'] = 0
