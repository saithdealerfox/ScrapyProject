...
for record in range(0, num_customers):
    ...
    temp_table = WebDriverWait(browser, 10).until(EC.element_to_be_clickable((By.XPATH, "//a[@id='DataPanel_ContentMain_CustomerName_{}']".format(record))))
    temp_table.click()
    if len(browser.window_handles) >0:
        browser.switch_to.window(browser.window_handles[1])
        time.sleep(2)
        browser.execute_script("var i = document.getElementById('OpportunityPanel_ViewPrevOpptyLink'); if (i != null) {i.click()};")
        ...
        source_detail_panel = browser.find_elements_by_xpath("//*[@id='OpportunityPanel_ActiveOpptyPanel']/table/tbody/tr[7]/td")
        ...
        #    x= {'L':src_list[source_detail]['L']+1}
        #    src_list[source_detail]['L']+=1
        # else:
        #    x = {'L' : 1}
        #    src_list[source_detail]= x
